...
public class TestRestIndexPageContent extends ITestImpl {
    ...
    @JTestClass.pre("xxx")
    @JTestClass.step("post http://127.0.0.1:80/rest/indexPageContent")
    @JTestClass.exp("ok")
    ...
    @JTestClass.pre("xxx")
    @JTestClass.step("post http://127.0.0.1:80/rest/indexPageContent/previewImage")
    @JTestClass.exp("ok")
    public void test0003_previewImage() {
        MsgRequest req = new MsgRequest();
        req.msgBody.put("documentId", 771);
        String ret = httpclient.post("http://127.0.0.1:80/rest/indexPageContent/previewImage", JSONObject.toJSONString(req), "application/json");
        httpclient.checkStatusCode(200);
        MsgResponse resp = MsgResponse.parseResponse(ret);
        checkEQ(0, resp.getRetcode());
        System.out.println("======post request======");
